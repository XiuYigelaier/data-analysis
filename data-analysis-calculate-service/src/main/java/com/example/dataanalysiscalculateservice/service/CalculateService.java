...
public class CalculateService {
    ...
    String NATION_FILE_PATH = "D:\\javaProject1\\DataAnalysis\\data-analysis-calculate-service\\src\\main\\resources\\NationRules";
    String AREA_FILE_PATH = "D:\\javaProject1\\DataAnalysis\\data-analysis-calculate-service\\src\\main\\resources\\AreaRules";

    public TalentRankTrans calculate(String developerName) throws Exception {

        Set<String> languages = new HashSet<>();
        BigDecimal calculateSum = BigDecimal.ZERO;
        Map result = (Map) calculateClientFeign.graphqlSearch(developerName).getData();
        ...
        talentRankTrans.setName(developerName);
        ...
        for (LinkedHashMap pullRep : pullRequestReviewContributionsByRepositoryList) {
            ...
            calculateEntity.setReposCommitsCount((Integer) ((Map) rep.get("commitComments")).get("totalCount"))
                    .setReposWatcherCounts((Integer) ((Map) rep.get("watchers")).get("totalCount"))
                    .setReposIssuesCount((Integer) ((Map) rep.get("issues")).get("totalCount"))
                    .setReposStarCount((Integer) rep.get("stargazerCount"))
                    .setPullReviewForReposCount((Integer) ((Map) pullRep.get("contributions")).get("totalCount"));
        ...
        for (LinkedHashMap issueRep : issueContributionsByRepositoryList) {
            ...
            calculateEntity.setReposCommitsCount((Integer) ((Map) rep.get("commitComments")).get("totalCount"))
                    ...
                    .setReposIssuesCount((Integer) ((Map) rep.get("issues")).get("totalCount"))
                    .setReposStarCount((Integer) rep.get("stargazerCount"))
                    .setIssuesForRepsCount((Integer) ((Map) issueRep.get("contributions")).get("totalCount"));
            calculateSum = calculateContributions(calculateEntity).add(calculateSum);
            languages.add((String) ((Map) rep.get("primaryLanguage")).get("name"));
        }
        ...
        for (LinkedHashMap commitRep : commitContributionsByRepositoryList) {
            ...
            calculateEntity.setReposCommitsCount((Integer) ((Map) rep.get("commitComments")).get("totalCount"))
                    ...
                    .setReposIssuesCount((Integer) ((Map) rep.get("issues")).get("totalCount"))
                    .setReposStarCount((Integer) rep.get("stargazerCount"))
                    .setCommitForReposCount((Integer) ((Map) commitRep.get("contributions")).get("totalCount"));
            calculateSum = calculateContributions(calculateEntity).add(calculateSum);
            languages.add((String) ((Map) rep.get("primaryLanguage")).get("name"));
        }
